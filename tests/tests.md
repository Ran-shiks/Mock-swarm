...
Nel paradigma della programmazione orientata agli oggetti (come è probabile che tu stia usando in Python), l'unità di base non è la funzione (come nel testing procedurale), ma la Classe, che combina dati (attributi) e operazioni (metodi).
...
Dato che stai lavorando in un contesto Agile, l'integrazione dovrebbe avvenire in modo incrementale, non con l'approccio Big-Bang (integrazione simultanea di tutti i moduli) che è sconsigliato perché rende difficile isolare la causa dei failures.
